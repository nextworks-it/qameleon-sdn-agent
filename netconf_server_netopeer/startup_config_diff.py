...
def find_diff(datastore_startup_xml, params_from_file, object_name):
	...
	#TODO There would be a set of minimal configuration that should be available at the start up time
	# Figure out what these are (for tpa, wss and so on) and set it when the below case occurs
	if not hasattr(startup_root_tmp,'child'):#2.1 File available, but no startup datastore 
...
def test_tpa():
	object_name="tpa"
	datastore_filename="datastore-"+object_name+".xml";
	params_from_file=get_configuration_from_file_tpa(object_name+"_conf.txt")
