...
class SettableRpc implements NetconfOperation {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SettableRpc.class);
    ...
    public Document handle(final Document requestMessage, final NetconfOperationChainedExecution subsequentOperation)
        ...
        LOG.info("Document requestMessage class name: "+requestMessage.getClass().getName());
        LOG.info("message into SettableRpc: "+XmlUtil.toString(requestMessage));
        LOG.info("\n\n");

        final XmlElement requestElement = XmlElement.fromDomDocument(requestMessage);
        LOG.info("Settable RPC");
        ...
        requestMessage.getDocumentElement().setAttribute(XmlNetconfConstants.MESSAGE_ID, msgId);

        Document customDocument = new DocumentImpl();
        customDocument.createElement(XmlNetconfConstants.OK);
        //return customDocument;

        final Optional<Document> response = rpcHandler.getResponse(rpcElement);

        LOG.info("RPC handler class name: "+rpcHandler.getClass().getName());
        LOG.info("class of subsequentOperation is "+subsequentOperation.getClass().toString());
